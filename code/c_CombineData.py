...
def AddStream(data):
    stream_data = pd.read_csv('../data/stream/nomis-20230320-0855-db.csv')
    ...
    stream_data = stream_data.rename(columns={'lat_sp [DD]':'latitude', 
                                            ...
                                            'chla [ug g-1]' : 'chla'})
    stream_data['slope'] = math.degrees(math.atan(stream_data['gl_elevation_diff'] / stream_data['gl_distance']))
    stream_data['abs_latitude'] = abs(stream_data['latitude'])
    stream_data['rel_elevation'] = 1 / (stream_data['elevation'] / stream_data['abs_latitude'])
    stream_data = stream_data[['Sample','latitude','longitude','elevation','slope','rel_elevation',
                            'pc_water_temp','pc_ph','pc_conductivity','pc_turbidity',
                            'nut_din','nut_srp','bacterial_abundance','chla']]
    
    for var in ['latitude','longitude','elevation','slope','rel_elevation','pc_water_temp','pc_ph','pc_conductivity','pc_turbidity',
                'nut_din','nut_srp','bacterial_abundance','chla']:
