...
def GetGlacierData(stream_data):
    for i, r in stream_data.iterrows():
        ...
        for ssp in ssps:
            ...
            future_area = pd.read_csv(f'RawData/Glaciology/Area_Dist_to_term/RGIreg{region}_{ssp}_Area.dat', 
                                    skiprows = 0, delim_whitespace=True,
                                    dtype = area_dtypes)
            future_dist = pd.read_csv(f'RawData/Glaciology/Area_Dist_to_term/RGIreg{region}_{ssp}_Dist_to_terminus.dat', 
